...
signed main() {
    ...
    // best L , R ===>? L = 0 , R = n - 1;
    priority_queue<array<ll , 3>>s;// sum , L , R;
    s.push(array<ll , 3>{accumulate(v , v + n , 0LL) , 0 , n - 1});
    vector<ll>res;
    map<pair<int , int> , bool>seen;
    while(res.size() != k) {
        auto cur = s.top();
        s.pop();
        if(seen.count(make_pair(cur[1] ,cur[2])))continue;
        seen[make_pair(cur[1] , cur[2])] = true;
        res.emplace_back((cur)[0]);
        auto [sum , l , r] = cur;
        s.push(array<ll , 3>{sum - v[l] , l + 1 , r});
        s.push(array<ll , 3>{sum - v[r] , l , r - 1});
    }
    for(auto &val:res) {
        cout<<val<<" ";
    }
