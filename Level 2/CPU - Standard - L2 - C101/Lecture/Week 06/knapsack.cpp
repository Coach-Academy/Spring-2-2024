...
/*==============================================  KHALWSH  ==============================================*/
pair<int,int>items[N];//item[i].first: weight of the ith item , item[i].second: value of the ith item
int solve(int idx , int W){
	if(W < 0)return -1e9;
	if(idx == n){
		return 0;
	}
	return max(solve(idx + 1 , W) , solve(idx + 1 , W - items[idx].first) + items[idx].second);
}
signed main() {
	khaled
	//knapsack problem: a theif want to steal your house and he has bag can carry M units of weight 
	//[weight , money]
