...
int pref[N];
int mx[N];
map<pair<int , int> , bool>fail;
void dfs(int node , int par , int smash) {
    if(fail[{min(par , node) , max(par , node)}])
        pref[node] = smash;
    mx[node] = smash;
    for(auto &val:adj[node]) {
        if(val.first == par)continue;
        dfs(val.first , node , smash + fail[{min(node , val.first) , max(node , val.first)}]);
        mx[node] = max(mx[node] , mx[val.first]);
    }

}
signed main() {
    khaled
    // any node has pref[i] == max[i] : res++
    int n;cin>>n;
    for(int i = 0;i < n - 1;i++) {
        int a , b , c;cin>>a>>b>>c;
        a-- , b-- ;
        ...
        fail[make_pair(min(a , b) ,max(a , b))] = (c == 2 ? 1 : 0);
    }
    dfs(0 , -1 , 0);
    int res = 0;
    for(int i = 0;i < n;i++) {
        if(pref[i] == mx[i] && mx[i] != 0) {
            res++;
