...
signed main() {
    ...
    while (t--) {
        ...
        bool found = false;
        multiset<int> copy(b , b + n);
        for (int i = 0; i < m; i++) {
            if (ms.find(d[i]) != ms.end())
                ms.erase(ms.find(d[i]));
            else {
                if (ms.empty() && copy.find(d[i]) == copy.end())found = true;
                else if (copy.find(d[i]) != copy.end() && ms.empty())found = false;
            }
        }
        cout << (ms.empty() && !found ? "YES" : "NO") << line;
    }
}
