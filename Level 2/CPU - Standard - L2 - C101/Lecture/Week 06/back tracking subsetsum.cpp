...
vector<int>subset;
// 1 1 0  1 1  0 0 0 0 0
void solve(int idx = 0 , int summation = 0){
	if(idx == n){
		if(summation == sum){
			for(auto &val:subset)
				cout<<val<<" ";
	...
	solve(idx + 1 , summation + arr[idx]);
	//leave
	subset.pop_back();
	solve(idx + 1 , summation);
...
signed main() {
	...
	cin>>sum;
	solve();
    // given n <= 20 sum = x print all subsets that has summation(subset) = x
    // 2 4 5 5 1 7 1 11   sum = 15
}
