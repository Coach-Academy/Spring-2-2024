...
/*==============================================  KHALWSH  ==============================================*/
string s;
ll freq[3] , need[3] , cost[3];
ll m;
bool can(ll mid){
	ll needb = (need[0] == 0 ? 0 : need[0] * mid);
	ll needc = (need[2] == 0 ? 0 : need[2] * mid);
	ll needs = (need[1] == 0 ? 0 : need[1] * mid);
	needb -= freq[0];
	needs -= freq[1];
	needc -= freq[2];
	needb = max(needb , 0ll);
	needc = max(needc , 0ll);
	needs = max(needs , 0ll);
	return cost[0] * needb + cost[1] * needs + cost[2] * needc <= m;
}
signed main() {
	khaled
	cin>>s;
