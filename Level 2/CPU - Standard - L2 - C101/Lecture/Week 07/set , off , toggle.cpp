...
/*==============================================  KHALWSH  ==============================================*/\
int set_bit(int x , int ith) {
	// set the ith bit in x and return x
	return x | (1<<ith);
}
int off_bit(int x , int ith) {
	// off the ith bit in x and return x
	return x & (~(1<<ith));
}
int toggle_bit(int x , int ith) {
	// toggle the ith bit and return x
	return x ^ (1<<ith);
}
bool check(int x , int ith) {
	//check if x % (1<<ith) == 0
	return (x & ((1<<ith) - 1)) == 0;
}
signed main() {
	khaled
    cout<<boolalpha<<check(16 , 2); // if 5 % 4 == 0
}
